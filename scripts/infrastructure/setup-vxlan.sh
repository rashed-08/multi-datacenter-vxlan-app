...
# DC
sudo ip link add vxlan${VXLAN_DC1_ID} type vxlan id $VXLAN_DC1_ID dev $PHYSICAL_INTERFACE dstport $VXLAN_PORT local ${HOST_IP} || true
sudo ip addr add $VXLAN_DC1_GATEWAY/16 dev vxlan${VXLAN_DC1_ID} || true
sudo ip link set vxlan${VXLAN_DC1_ID} up

# DC2
sudo ip link add vxlan${VXLAN_DC2_ID} type vxlan id $VXLAN_DC2_ID dev $PHYSICAL_INTERFACE dstport $VXLAN_PORT local ${HOST_IP} || true
sudo ip addr add $VXLAN_DC2_GATEWAY/16 dev vxlan${VXLAN_DC2_ID} || true
sudo ip link set vxlan${VXLAN_DC2_ID} up

# DC3
sudo ip link add vxlan${VXLAN_DC3_ID} type vxlan id $VXLAN_DC3_ID dev $PHYSICAL_INTERFACE dstport $VXLAN_PORT local ${HOST_IP} || true
